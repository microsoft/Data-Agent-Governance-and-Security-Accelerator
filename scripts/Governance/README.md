...
### 0. Provide Resource Names (Required)

Edit `infra/main.bicepparam` and replace the placeholder values with the names of your existing resources (Purview account, AI Foundry project, Azure OpenAI instance, etc.). Provide subscription IDs as needed (`purviewSubscriptionId`, `aiSubscriptionId`) to handle cross-subscription scenarios. During `azd up`, a pre-provision hook copies these values into the azd environment for the PowerShell scripts to consume.

To sync manually, run:

```bash
pwsh ./scripts/Governance/apply-governance-parameters.ps1 -EnvironmentName dev -ParametersFile ./infra/main.bicepparam
```

If you prefer to maintain a JSON file instead, update `scripts/Governance/governance.parameters.json` and apply it with:

```bash
pwsh ./scripts/Governance/apply-governance-parameters.ps1 -EnvironmentName dev
```

Only non-empty, non-placeholder entries are written to the azd environment. Re-run the helper script anytime you change the parameter file.
